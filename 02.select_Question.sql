...
>>없음,  테이블 확인해봤는데 해당일에 입사한 사람 없음.
>>문제를 81/09/08로 바꿔도 좋을듯
*/
...
select *
from emp
where hiredate between '1981-04-02' and now();

-- 부등호로 <= 표현 가능

-->> /보다는 -권장 yy/mm/dd 보다는 yyyy-mm-dd
/**7.
...
select *
from emp
where hiredate not between '81/12/09' and now();
...
from emp
where not deptno = 20 and not deptno = 30;
-- not in 사용
